...
export class EditclientComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.clientService.getClient(this.idClient).subscribe(
      client =>{this.clientFormGroup = this.fb.group({
        ...
        cp: [client.cp, Validators.compose([Validators.required,
  ...
  onUpdateClient(): void {
    ...
    this.clientService.updateClient(this.clientFormGroup?.value).subscribe(data => {alert('maj ok')},
      err => {
  ...
  onSeeComfacts() {
    ...
    this.comfactService.getComfactsClient(this.idClient).
    subscribe(data => {this.comfacts=data},
      err => {
        alert(err.headers.get("error"));
      });
  }
